...
const VideoConvert = () => {
  ...
  const handleFileUpload = (info) => {
    const newFiles = info.fileList
      ...
      .filter(Boolean)
      .filter((file) => {
        // Check for file size limit (1 GB)
        if (file.size > 1024 * 1024 * 1024) {
          message.info("File size exceeds 1 GB. Please upload a smaller file.");
          return false;
        }
        return true;
      });
    ...
    const updatedFiles = newFiles.map((newFile) => {
      ...
      const defaultFormat = videoOptions.includes(fileExtension)
        ? fileExtension
        : "";
      ...
      return {
        ...
        downloadUrl: "",
        progress: 0,
        format: defaultFormat, // Auto-detected format
      };
    });
        ...
        {files.map((fileObj) => (
          ...
          >
            ...
            <div className="flex gap-2">
              <Select
                value={outputFormat[fileObj.filename] || fileObj.format || ""}
                onChange={(value) =>
